...
<template>
  <NuxtLayout name="full-width">
    <div v-if="home" class="bg-ctp-base text-ctp-text" style="font-size: 1.25rem; line-height: 1.6;">
      <!-- Hero Section -->
      <header style="margin-bottom: 4rem;">
        <h1 style="font-size: 4rem; line-height: 1; margin-bottom: 1rem; font-weight: 800; letter-spacing: -0.05em;" class="text-ctp-maroon">
          {{ home.hero.title }}
        </h1>
        <p style="font-size: 1.75rem; font-weight: 700; margin-bottom: 2rem;" class="text-ctp-mauve">
          {{ home.hero.subtitle }}
        </p>
      ...
      <section v-for="section in home.sections" :key="section.id" style="margin-bottom: 4rem;">
        <h2 style="font-size: 2.5rem; font-weight: 800; margin-bottom: 1.5rem; border-bottom: 4px solid var(--ui-primary); display: inline-block;" class="text-ctp-maroon">
          {{ section.title }}
        </h2>
        ...
        <div v-if="section.type === 'text'">
          ...
          <ul v-if="section.list" style="list-style: none; padding: 0; margin-top: 2rem;">
            <li v-for="item in section.list" :key="item.title" style="margin-bottom: 1.5rem;" class="flex items-start">
              <span class="mr-3 mt-1 inline-flex items-center justify-center px-2 py-0.5 rounded text-xs font-bold bg-ctp-mauve text-ctp-base">
                {{ item.title }}
              </span>
              <span class="text-ctp-text">{{ item.text }}</span>
            </li>
          </ul>
          <p v-if="section.quote" style="font-weight: 700; margin-top: 2rem; font-size: 1.5rem; text-align: center; font-style: italic;" class="text-ctp-subtext0">
            "{{ section.quote }}"
          </p>
        </div>
        ...
        <div v-else-if="section.type === 'infrastructure'">
          <h3 style="font-size: 1.75rem; margin-bottom: 1rem;" class="text-ctp-subtext0">{{ section.subtitle }}</h3>
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 2rem; text-align: center;">
              <thead>
                <tr class="text-ctp-subtext0">
              ...
              <tbody>
                <tr v-for="item in section.items" :key="item.name">
                  ...
                  <td style="padding: 12px; vertical-align: middle;">
                    <a :href="item.url" style="font-weight: 700; text-decoration: none;" class="text-ctp-blue hover:text-ctp-mauve hover:underline">
                      <Icon v-if="item.icon" :name="item.icon" class="size-6 inline-block mr-2" />
                      {{ item.name }}
                    </a>
                  </td>
                  <td style="padding: 12px; vertical-align: middle; font-size: 1rem;" class="text-ctp-subtext1">
                    {{ item.function }}
                  </td>
                </tr>
              </tbody>
...
v-for="item in section.items" :key="item.name" 
              ...
              <h4 style="margin: 0 0 0.5rem 0; font-size: 1.25rem;">
                <a :href="item.url" style="text-decoration: none;" class="text-ctp-blue hover:text-ctp-mauve hover:underline">
                  {{ item.name }}
                </a>
              </h4>
              <p style="margin: 0; font-size: 0.95rem;" class="text-ctp-subtext1">{{ item.description }}</p>
            </div>
          </div>
        </div>
      ...
      <!-- Footer -->
      <footer style="text-align: center; border-top: 1px solid var(--tw-prose-borders); padding-top: 2rem; font-size: 0.875rem;" class="text-ctp-subtext1">
        <p>
          Last updated: {{ home.footer.lastUpdated }} | 
          <a :href="home.footer.statusUrl" class="text-ctp-blue hover:text-ctp-mauve hover:underline">Detailed System Status</a>
        </p>
      </footer>
    </div>
  </NuxtLayout>
