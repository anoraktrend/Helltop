...
[**Git (Forgejo)**](https://git.helltop.net) <status-indicator name="git" /> - I host my own git server, in the long shot chance Codeberg sinks. (it's super slow, but I enjoy using it, you can join with discord!)

[**Cloud (Nextcloud)**](https://cloud.helltop.net) <status-indicator name="cloud" /> - My Nextcloud Instance. Why use google's BS when you can host your own with even more and better features? Nextcloud does almost everything I need there. (usage is restricted to trusted friends, you know where to go if you wanna get access.)

[**LDAP (lldap)**](https://ldap.helltop.net) <status-indicator name="ldap" /> - Lightweight LDAP server for unified authentication across my services.

[**Authelia**](https://auth.helltop.net) <status-indicator name="auth" /> - Single Sign-On and 2FA for my entire network.

[**Jellyfin**](https://jellyfin.helltop.net) <status-indicator name="jellyfin" /> - My Jellyfin instance, the beating heart of my media library. Streaming services got you down? Own a bunch of DVDs, Blurays, and CDs? Rip all of them to your local storage solution and host your own! I'll have to make a blog post talking about how to do that with Tailscale as your LAN<->WAN access tool. (usage is restricted to trusted friends, you know where to go if you wanna get access.)

[**Seerr**](https://seerr.helltop.net) <status-indicator name="seerr" /> - Requests management for my media stack.

[**Immich**](https://immich.helltop.net) <status-indicator name="immich" /> - My Immich! Remember how I said Nextcloud does *Almost* everything I need from google? Well, this is the one that does the one other thing I was missing... *good* photo library management. Back up everything to it from your phone using the app, and vis versa.
